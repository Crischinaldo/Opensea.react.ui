...
const NFTNavigation = (props) => {
  ...
  const navItems = props.config.nftItems.map((item, idx) =>
  ...
  return (
    <Navbar
      className={"fixed-top-under nft-nav bg-white header-bottom " + expand}
      color-on-scroll="100"
      expand="lg"
    >
      <Container>
      <div className="navbar-translate header-logo" id={expandLogo}>
        ...
        >
          ...
          <Collapse isOpen={collapseOpen} className="bg-gray-smoke" navbar>
          <Nav navbar>
            {navItems}
          </Nav>
        </Collapse>
